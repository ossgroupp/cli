...
async function verifyTokens({ id, secret }) {
	const callPIM = createAPICaller('https://pim.ossgroup.com/graphql', {
		headers: {
			'X-OSSPIM-Access-Token-Id': id,
			'X-OSSPIM-Access-Token-Secret': secret,
		},
	});
	try {
		await callPIM({
...
function GetAccessTokens({ onDone }) {
	...
	if (existingTokens.current && !verifiedExistingTokens) {
		return (
			<Box>
				<Text>Verifying existing OSSPIM Access Tokens...</Text>
			</Box>
		);
	}
	...
	if (!askForTokens && existingTokens.current) {
		return (
			<Box flexDirection="column" key="access-tokens-found-existing">
				<Text>Found existing OSSPIM Access Tokens. Want to use it?</Text>
				<Select
					compact
					onChange={(answer) => {
						if (answer.value === 'yes') {
	...
	return (
		<>
			<Box flexDirection="column" marginBottom={1}>
				<Text>
					Please provide Access Tokens to bootstrap the instance
					<Newline />
					<Text dimColor>
						Learn about access tokens:
						https://ossgroup.com/learn/developer-guides/access-tokens
					</Text>
				</Text>
			</Box>
			{invalidKeys && (
