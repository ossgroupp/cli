...
const { stepBootstrapInstance, RunBootstrapper } = importJsx(
	'../_shared/step-bootstrap-instance.js'
);
...
function InstallDepsAndBootstrap(props) {
	...
	const [bootstrappingDone, setBootstrappingDone] = React.useState(
		props.answers.bootstrapInstance === 'no'
	);
		...
		<>
			...
			{props.answers.bootstrapInstance !== 'no' && (
				<Box flexDirection="column" marginBottom={1}>
					<Box marginBottom={1}>
						<Text>Bootstrapping {props.answers.instance}</Text>
					</Box>
					<RunBootstrapper
						{...props}
...
const steps = [
	{
							...
							},
							{
								...
								render: (
									<>
										...
										<Newline />
										<Text dimColor>
											Demo: https://osspim-gatsby-boilerplate.vercel.app/
										</Text>
									</>
								),
							},
		...
		answer({ answers, answer }) {
			...
			if (answer.value === 'nextjs-content-commerce') {
				answers.defaultInstance = 'voyage';
			} else if (answer.value === 'nextjs-subscription-commerce') {
				answers.defaultInstance = 'photofinder';
			} else if (answer.value === 'nextjs-conference') {
				answers.defaultInstance = 'conference-boilerplate';
			}
		},
		staticMessage(props) {
			return (
	...
	},
	{
		render({ resolveStep, answers }) {
			return (
				<>
					<Text>
						Please select a OSSPIM instance
						<Newline />
						<Text dimColor>
							Don't have an instance yet? Create one at
							https://ossgroup.com/signup
						</Text>
					</Text>
					<Select
						onChange={(answer) => resolveStep(answer)}
						options={[
							{
								value: answers.defaultInstance,
								render: (
									<>
										<Text>Our demo instance ({answers.defaultInstance})</Text>
										<Newline />
							...
							{
								value: '[use-own-instance]',
								render: (
									<>
										<Text>My own instance</Text>
									</>
								),
							},
						]}
	...
	},
	{
		render({ resolveStep, answers }) {
			return (
				<Box>
					<Box marginRight={1}>
						<Text>Enter your instance identifier:</Text>
					</Box>
					<UncontrolledTextInput
						placeholder={answers.defaultInstance}
		...
		answer({ answers, answer }) {
			...
			answers.useOwnInstance = true;
		},
		when({ answers }) {
			return answers.instance === '[use-own-instance]';
		},
	},
	{
		staticMessage(props) {
			return (
				<Text>
					Using OSSPIM instance{' '}
					<Text color={highlightColor}>{props.answers.instance}</Text>
				</Text>
			);
		},
	},
			...
			return (
				<Box flexDirection="column" marginTop={2}>
					...
					<Box flexDirection="column" marginBottom={2}>
						...
						<Text>Reach us at: https://ossgroup.com/about#contact</Text>
					</Box>
				</Box>
			);
		},
